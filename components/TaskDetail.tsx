...
export default function TaskDetail({ task }: TaskDetailProps) {
  ...
  const handleCreateOT = () => {
    ...
    params.set("desc", String(task.description || task.task_description || ""));
    params.set("item", String(task.item_description || ""));
    params.set("code", String(task.code || ""));
    params.set("duration", String(task.duration || 0));
    params.set("type", String(task.tasks_types_main_description || task.type_description || ""));
    params.set("date", String(task.date_maintenance || ""));
    params.set("plan", String(task.group_task_description || ""));
    params.set("priority", String(task.priorities_description || "Media"));
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "#94a3b8", gap: "16px" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <p style={{ fontSize: "16px", fontWeight: 600, color: "#475569", margin: 0 }}>Selecciona una tarea pendiente</p>
          <p style={{ fontSize: "13px", color: "#94a3b8", margin: "6px 0 0" }}>Elige una tarea de la columna izquierda para ver sus detalles</p>
        </div>
      </div>
    );
  }
  ...
  const priorityLabel = PRIORITY_LABELS[task.id_priorities] || String(task.priorities_description || "MEDIA");
    ...
    <div style={{ padding: "24px", height: "100%", overflowY: "auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: "15px", fontWeight: 700, color: "#1e293b" }}>
          {task.description || task.task_description || "Sin descripcion"}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "16px" }}>
        {(task.tasks_types_main_description || task.type_description) && (
          <span style={tagStyle("#dcfce7", "#166534")}>{String(task.tasks_types_main_description || task.type_description || "").toUpperCase()}</span>
        )}
        {task.tasks_types_description && (
          <span style={tagStyle("#dbeafe", "#1e40af")}>{String(task.tasks_types_description).toUpperCase()}</span>
        )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "14px" }}>
        ...
        <DetailCard icon="⏱" label="Duracion estimada" value={formatDuration(task.duration)} />
        {overdue && <DetailCard icon="⚠️" label="Dias de atraso" value={task.delay + " dias"} alert={true} />}
        {task.wo_folio && <DetailCard icon="📋" label="Folio OT" value={String(task.wo_folio)} />}
      </div>

      {task.location_description && (
        <div style={sectionStyle}>
          <div style={labelStyle}>UBICACION</div>
          <div style={{ fontSize: "13px", color: "#475569" }}>{String(task.location_description)}</div>
        </div>
      )}
      ...
      {task.workshop_description && (
        <div style={sectionStyle}>
          ...
          <div style={{ fontSize: "13px", color: "#4f46e5", fontWeight: 600 }}>{String(task.workshop_description)}</div>
        </div>
      )}
      ...
      {task.requested_by && (
        <div style={sectionStyle}>
          ...
          <div style={{ fontSize: "13px", color: "#475569" }}>{String(task.requested_by)}</div>
        </div>
      )}
